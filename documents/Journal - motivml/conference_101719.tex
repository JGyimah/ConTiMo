...
\begin{abstract}
Recent technological advancements have led to a growing demand for customized solutions, that can operate under non-deterministic conditions, with a high level of efficiency.
In the wake of the current pandemic, industries have had to deal with unforeseen challenges in the form of labour, production and logistic deficits. These challenges have negatively impacted human-driven supply systems, due to incidental safety regulations and human resource depletion. Undeniably, these challenges have had a resounding effect across the global service industry and as such have compelled industries to devise alternative methods to alleviate them. One of such alternatives which has emerged and proven to be feasible and useful in practice, is the implementation and deployment of service robots.
...
Valid combinations of robotic features to fit varying contexts, have the propensity to give rise to some level of variability---i.e., the ability of a software artifact to be changed to fit multiple contexts, environments, or purposes. This phenomenon, presents the need for a possible strategy that enables roboticists to adapt, customize or configure robotic applications in correspondence to different operational scenarios. This has also provided the much needed incentive to have an effective mechanism that allows domain experts to plan, design, and implement variability.
...
To solve this problem, we applied design science methods, to define example systems while studying their existing variabilities, all in an attempt to implement a variability modelling framework, that provides a language together with mechanisms, capable of managing variability with respect to a feature's binding time and mode. Our open source solution provides basic support for binding features, as well as evidence of the extensibility of robotics reference architectures, to support variability, in a domain where variability is typically performed in an ad hoc manner. That being said, this study is expected to alleviate extension complexities, increase binding flexibility, and provide mechanisms for managing variability in robotic systems.

Furthermore, this study provides evidence to back the claim that our proposed variability management technique is novel, realizable, useful in practice and has the capability of assessing valid configurations.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textit{The lack of mechanisms and guidelines for implementing variability management techniques in robotic systems:} Even though a plethora of techniques exist, there are no specific guidelines that detail which technique to use and how to use it within the robotics domain.
    ...
    In a domain like robotics which is relatively new and rapidly expanding, when it comes to implementing variability mechanisms, there is no streamlined and platform specific means of doing so. In many cases, robotic application developers resort to ad hoc means of implementing variability.

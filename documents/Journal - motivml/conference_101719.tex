...
\begin{IEEEkeywords}
model-driven software engineering, variability, service robots, robotics, domain specific languages, feature reconfiguration, flexible feature binding
\end{IEEEkeywords}
...
\begin{enumerate}
    ...
    \begin{enumerate*}[label=(\roman*)]
    	\item verify the specifications that define their capabilities and
    	\item provide comprehensive coverage of the application and its platform, along with practical step-by-step usage of its functionalities.
    \end{enumerate*}
...
Our research method of choice for this study is design science. We chose the design science paradigm mainly due to its deep roots in engineering science. In that, it generally offers a problem-solving approach that seeks to create innovations that define the ideas, practices, technical capabilities, and products through which analysis, design, implementation, management, and information system usage can be effectively and efficiently accomplished  \cite{des-res}. Figure \ref{methover} provides a layered and well defined overview of the empirical steps we took to create artefacts that rely on existing theories that are applied, tested, modified, and extended through experience, creativity, intuition, and problem solving, as part of our study.
...
\centering
\includegraphics[height=8cm, width=0.95\textwidth]{diagrams/methods-new.png}
\label{methover}
\end{figure*}
...
After defining both the concrete and abstract syntax of our language, we proceeded to establish the static semantics of our language. Static semantics in this context, refers to the underlying rules that determine the validity of model variants that can be created with our language. Key aspects of the static semantics which we clarified and established include:
\begin{enumerate}
    \item Outlining mechanisms that realise time and mode binding.
...
\begin{enumerate}
    ...
    \item Establish design decisions such as the general architecture of the system, dependency management, choice of technologies and their trade-offs, as well as implementation algorithms.
    \item Realise language designs with Python based algorithms and data structures. Likewise interfaces for integrating encapsulated feature implementations need to be realised in  C++\foot{https://isocpp.org/}.
...
\end{enumerate*}
\begin{itemize}
    \item \textit{\textbf{Correctness:}} The validity of models and configurations created with our language along with the implied effect of a model's feature constraints are closely evaluated by both a schema and a constraint checker. These operate by detecting syntax, semantic and constraint violations, and report them to the user accordingly.
    
    \item \textit{\textbf{Realizability:}} Proof of realizability indicates that our binding technique which forms the core of our implemented variability mechanisms is feasible. This is quite relevant, given that modelling variability with feature binding times and modes is yet to be applied in the robotics domain.
...
\begin{listing}[H]
\caption{Configuration Object Schema}
\begin{minted}[
framesep=2mm,
baselinestretch=1,
%bgcolor=LightGray,
...
An important use case of our language is that, it can be aid in developing mock-up examples that demonstrate our language's expressiveness. Within the context of this study, examples refer to modelled features of robot product lines. Figure \ref{ex1} and \ref{ex2} show graphical representations in the form of feature models, of selected example robots. Each feature present in these models, can be expressed textually with tokens from our language as shown in Listings \ref{feat-conc-text} and \ref{bind-conc-text}. Inspiration for these examples were drawn from general utility and disinfection robots such as TIAGo and UVD Model C.
...
Based on our understanding of the domain from previous steps, we extended our mock-up examples in Figure \ref{ex1} and \ref{ex2} against our set of already established language requirements. These requirements are as follows:
\begin{enumerate}
...
\end{enumerate}
Using the concrete textual syntax of our modelling language highlighted in Listing \ref{feat-conc-text} and \ref{bind-conc-text} as a reference point, we identified and generated example syntax representations that fulfil all the requirements listed above. Table \ref{tab:reqext} captures a mapping of each stated requirement along with its corresponding example syntax representation, that justifies and fulfils that same requirement.
...
The abstract syntax of any language refers to the logical representation of that language in memory. For any given feature model, let $\mathsf{Id}$ be a unique finite set of string labels,
$\mathsf{BTime}$ be the binding time of a feature and $\mathsf{BMode}$, the binding mode of a feature. Upon instantiation, a modelled robotic feature is selected by default. This is represented by a boolean variable $\mathbb{B}$, which is created for each feature, corresponding to a given feature's selected or unselected state in a configuration. In the instance that $\mathsf{FMo}$ is the set of all possible feature models, each model $\mathnormal{Mo} \in$ $\mathsf{FMo}$ is a set of features $\mathcal{F}$. Thus, $\mathsf{FMo}$ = $\mathcal{P}(\mathcal{F})$ where:
  
\begin{align*} 
	\mathcal{F} &= \mathsf{Id_{feature}} \times \lceil \mathsf{Id_{parent}} \rceil \times \mathcal{P}(\mathcal{\mathsf{Id_{children}}}) \times \mathbb{B} \times \lfloor Group \rfloor\\
	&\times \mathcal{P}(\mathcal{\mathsf{BTime}}) \backslash\{\emptyset\} \times \mathcal{P}(\mathcal{\mathsf{BMode}}\backslash\{\emptyset\}) \\\\
	&\mathsf{Group} = \{OR, XOR\}\\
	&\mathsf{BMode} = \{Static, Dynamic\}\\\
	&\mathsf{BTime} = \{Early, Late\}\\
	&\mathcal{P}(\mathcal{\mathsf{BTime}}) = \{\{Early\}, \{Late\}, \{Early, Late\}, \{\emptyset\}\}\\
	&\mathcal{P}(\mathcal{\mathsf{BMode}}) = \{\{Static\}, \{Dynamic\}, \{Static, Dynamic\}, \\
	&\{\emptyset\}\}\\\\
	&\mathsf{\lceil X \rceil = X \: U \: \{\top\}} \: and \: \mathsf{\lfloor X \rfloor = X \: U \: \{\bot\}}\\
	&\mathnormal{f} \in \mathcal{F} = \{\mathnormal{f} : \mathnormal{f} := (\mathsf{Id_{feature}}, \mathsf{Id_{parent}})\}\\
	& \mathnormal{f_{0}} = ("root", \top)\\
	& \mathnormal{f_{1}} = ("feature1", \bot)
\end{align*}

In Figure \ref{femet} and \ref{conmet} we provide abstractions to the user interfaces of both feature and configuration objects in the form of class diagrams. In both meta-models, sets of capability related data pertaining to the same concerns are modelled and encapsulated together. These related user-defined types can be instantiated with variable data values that conform to the requisite types defined in our abstract syntax. For instance,  By design, every modelled feature must bear a unique string \textit{id} attribute which serves as a reference point in key aspects of our framework. Every feature must also bear a \textit{name} attribute that describes its intended purpose and capability.
...
The configuration capabilities of a feature reflects its ability to adapt in accordance to variable contexts. Per our proposed variability management technique,
feature adaptations are determined by a binding time and binding mode combination, specified or assigned based on end user preferences. A configuration can only exists as long as a feature exists. Thus, each configuration bears reference to an existing feature by way of an \textit{id} attribute. This explains the existing compositional relationship between a feature and a configuration, as shown in Figure \ref{conmet}. 
...
\subsubsection{Semantic Domain Definition}
The semantic domain defines the set of all possible configurations. A configuration of a feature model is any model that consists of instantiated features. Let $\mathsf{Configs}$ be the set of all possible configurations and in our domain $\mathsf{D}$ and ($\mathsf{id}$, $\mathsf{mode }$, $\mathsf{time}$) $\in$ $\mathsf{D}$. Therefore:

\begin{align*}
	&\mathsf{id} \longrightarrow \lfloor \mathsf{Id} \rfloor\\
	&\mathsf{mode} \longrightarrow \lfloor \mathsf{BMode} \rfloor\\
	&\mathsf{time} \longrightarrow \lfloor \mathsf{BTime} \rfloor\\
	&\mathsf{Configs} = \mathsf{D}\\
\end{align*}

\subsubsection{Semantic Function Definition} Our resultant semantic function $\mathbb{S}$ maps any given feature model from the set of possible feature models to a set of valid configurations.

	\[\mathbb{S} : \mathsf{FMo} \longrightarrow \mathcal{P}(\mathsf{Configs})\]

\subsubsection{Identifying Binding Mechanisms}
Realising static, dynamic, early and late bindings in robotic applications can be tricky due to platform and language constraints. For both time and mode based binding, there are a number of optimised implementations and packages found in C++ and ROS, that can be used to realise this.

In Table \ref{tab:realmecha}, we have highlighted some of such implementations, otherwise known as mechanisms, which could potentially be used to realise all possible binding pairs found within our configuration space. These include, the C++ preprocessor or antenna for static, ROS pluginlib for dynamic and a combination of ROS parameters\foot{http://wiki.ros.org/rosparam} and roslaunch\foot{http://wiki.ros.org/roslaunch} for both early and late binding.
...
\begin{table}[H]
	...
	\begin{center}
		\begin{tabular}{c c}
			...
			Static & C++ preprocessor, antenna \\ \hline
			Dynamic &  ROS Pluginlib  \\ \hline
			Early &  roslaunch  and ROS parameters  \\ \hline
			Late &  Virtual functions, roslaunch and ROS parameters  \\ \hline
		\end{tabular}
		\label{tab:realmecha}
	\end{center}
\end{table}
...
As shown in the instance captured in Listing \ref{cpp-static} above, implemented feature classes \textit{Amcl.h}, \textit{MotionPlanningCtrl.h} and \textit{ComponentControl.h} will be included in our user program build if they are explicitly defined as part of the configuration. This mechanism exists strictly for static features that are loaded at compile time.
...
For specific definitions of features and their binding times and modes, one needs to be able to evaluate their existence in a configuration. Using propositional logic, the following denotations were derived with reference to Table \ref{tab:timoconf}.
...
\begin{center}
%\begin{tabular}{|c|c|c|}
\begin{tabularx}{125mm}{P{2cm} P{4cm} P{6cm}}
\hline
...
\includegraphics[width=10mm, height=12mm]{diagrams/mandatory.png} & A $ \Leftrightarrow $ B \\ \hline
    ...
    OR & A requires ($ B_1 \vee ... \vee B_n $) &  $ B_1 \vee ... \vee B_n \Leftrightarrow  A $ \\ \hline
    XOR (Alternative) & A requires ($ B_1 \oplus ... \oplus B_n $) &  $ (B_1 \vee ... \vee B_n \Leftrightarrow  A) \wedge \bigwedge \limits_{i < j} \neg(B_i \wedge B_j) $ \\ \hline
\end{tabularx}
\label{tab:propmap}
\end{center}
\end{table*}

The underlying logic of this language design requires the interaction of a feature's binding attributes, that serves as a variation mechanism for all feature models defined with this language. Table \ref{tab:timoconf} captures the semantics of possible binding time and binding mode combinations in any given model. This is relevant because, analysing the combined semantic effect of possible binding pairs in a model, gives insight into the feasibility or validity of combinations in the configuration space. Using propositional logic, Table \ref{tab:timoconf} shows the results of every possible interaction between binding pairs with respect to feature attributes such as mandatory, optional, includes, excludes, OR and XOR. 
...
\hline
    A & B & $ \neg$ B & A $ \Rightarrow $ $ \neg $ B & A $ \Rightarrow $ B & A $ \Leftrightarrow $ B \\\hline
         SE & SE & DL & 0 & 1 & 1 \\ \hline
         SE & SL & DE & 0 & 0 & 0 \\ \hline
         SE & DE & SL & 0 & 0 & 0 \\ \hline
         SE & DL & SE & 1 & 0 & 0 \\ \hline
         
         SL & SE & DL & 0 & 1 & 0 \\ \hline 
         SL & SL & DE & 0 & 1 & 1 \\ \hline
         SL & DE & SL & 1 & 0 & 0 \\ \hline
         SL & DL & SE & 1 & 0 & 0 \\ \hline
         
         DE & SE & DL & 0 & 1 & 0 \\ \hline
         DE & SL & DE & 0 & 0 & 0 \\ \hline
         DE & DE & SL & 0 & 0 & 0 \\ \hline
         DE & DL & SE & 1 & 0 & 0 \\ \hline
         
         DL & SE & DL & 0 & 1 & 0 \\ \hline
         DL & SL & DE & 0 & 1 & 0 \\ \hline
         DL & DE & SL & 1 & 0 & 0 \\ \hline
         DL & DL & SE & 0 & 0 & 0 \\ \hline
...
This paper mainly draws inspiration from research materials contributed by Sergio Garc\'{i}a, Claudio Menghi, Patrizio Pelliccione, Thorsten Berger, and Rebekka Wohlrab. This study particularly extends their work concerning SERA; A Self-adaptive dEcentralized Robotic Architecture for building autonomous, heterogeneous and collaborative robotic applications.
